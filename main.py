...
执行出口只有一个：读取事件级标记，由 on_decorating_result 把本条回复的
文本替换为 TTS 语音消息。

两个触发入口，均只负责打标，不自行合成语音：
    1. 用户明确要求用语音回复 —— LLM 读 reply_with_voice 的 docstring
       自行判断后调用该工具打标。
    2. 用户本条消息本身是语音输入（直接发送，或被 @ 时引用的消息是语音）
       —— on_llm_request 阶段检测到后直接打同一标记。
...
class NyaaVoicePlugin(Star):
    ...
    async def reply_with_voice(self, event: AstrMessageEvent) -> str:
        ...
        ## 说明
        用户以语音发送消息时，插件会自动激活语音回复，无需调用本工具；
        因此本工具只需处理"用户用文字明确要求语音"的情况。
        '''
        event.set_extra(_VOICE_REPLY_FLAG, True)
        logger.info(
